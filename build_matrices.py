...
def build_base_calls_matrix(file_s, all_SNVs, barcodes):
    ...
    for snv in all_SNVs:
        ...
        multibase = []   # record all the SNP positions with more than 1 base, to be removed from the matrices
        if len(snv.REF) == 1 & len(snv.ALT) == 1:   # keep SNP only
            ...
            multibase.append(position)

    ref_base_calls_mtx.drop(multibase)
    alt_base_calls_mtx.drop(multibase)
...
def main():
    ...
    for record in vcf.Reader(open(file_v, 'r')):
        if record.samples[0]['GL'][1] > math.log10(1-epsilon):   # ignore SNVs with homozygous genotypes
